...
class Login(QDialog):
    ...
    def widgets(self):
        ...
        header.setStyleSheet(
            "background: qlineargradient(spread:pad, x1:0, y1:0, x2:0, y2:1, stop:0 #0d47a1, stop:1 #4a148c)"
        )
        ...
        backgroundLogin.setStyleSheet(
            "QFrame {border-radius: 20px; background-color: white; margin-left: 30px;  margin-right: 30px}"
        )
        ...
        userL.setStyleSheet(
            "background-color: rgba(4,28,56, 0);font: 18px;margin-left: 30px; margin-right: 30px; color: #311b92; font-weight:bold"
        )
        self.user = QLineEdit()
        self.user.setStyleSheet(
            "font:14px; margin-left: 30px; margin-right: 30px; padding: 5px"
        )
        ...
        self.iniciar.setStyleSheet(
            "QPushButton {background-color:#4a148c; color: white; border-radius: 10px; font: 15px bold; font-weight:bold; margin: 10px 50px 10px 50px ; padding: 7px;}"
    ...
    def submit(self):
        if self.user.text() == os.getenv(
            "USER_ENV"
        ) and self.password.text() == os.getenv("PASS_ENV"):
            self.close()
