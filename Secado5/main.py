...
class Estacion(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet(
            "QMainWindow {background: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 #9e9e9e, stop:1 #707070)}"
        )
        self.centerWindow()
        # signals
        ...
        self.data = provider.Data(
            ...
            self.zone3.humValue,
            self.zone4.tempValue,
            self.zone4.humValue,
            self.zone5.tempValue,
            self.zone5.humValue,
            self.humGrain.text,
        )
    ...
    def centerWindow(self):
        ...
        self.move(
            sizeScreen.x() - int(sizeWindow.width() / 2),
            sizeScreen.y() - int(sizeWindow.height() / 2),
        )
    ...
    def createWidgets(self):
        ...
        self.zone4 = widgets.EnvironmentWidget("Zona 4", "17 °C", "50 %")
        self.zone5 = widgets.EnvironmentWidget("Zona 5", "17 °C", "50 %")
        self.humGrain = widgets.RoundedContainerInput(
            "Humedad Grano", "humedadpurpleSmall.svg", "50 %"
        )

        # Menu de visuzalización
        self.menuVis = widgets.MenuVisualizacion()
        self.menuVis.listaVariables.currentIndexChanged.connect(
            self.signals.signalUpdateGraph.emit
        )
        # Grafica
        self.graph = widgets.Graph()
        self.toolbarFig = NavigationToolbar(self.graph.figure, self)
        self.toolbarFig.setStyleSheet(
            "background:rgba(45,208,179,0); color:white; font:20; font-weight: bold"
        )
        ...
        gridLayout.addWidget(self.zone4, 0, 4)
        gridLayout.addWidget(self.zone5, 0, 5)
        gridLayout.addWidget(self.humGrain, 1, 0)
        gridLayout.addWidget(self.menuVis, 1, 5)
    ...
    def updateGraph(self):
        ...
        self.plot.plot(
            self.data.getTime(indexActual),
            self.data.getData(indexActual),
            variableActual,
            indexActual,
        )
...
class Thread(QThread):
    ...
    def run(self):
        ...
        schedule.every(int(self.samplingTimes["env4"])).seconds.do(self.data.env4)
        schedule.every(int(self.samplingTimes["env5"])).seconds.do(self.data.env5)
