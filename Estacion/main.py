...
class Estacion(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet(
            "QWidget {background: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 #9e9e9e, stop:1 #707070)}"
        )
        self.centerWindow()
    ...
    def createWidgets(self):
        # Datos Actuales
        self.temperaturaWidget = widgets.RoundedContainer(
            "Temperatura", "temperatura.svg", "25 °C"
        )
        self.humedadWidget = widgets.RoundedContainer("Humedad", "humedad.svg", "50 %")
        self.irradWidget = widgets.RoundedContainer(
            "Irradiancia", "irrad.png", "200 W/m²"
        )
        self.velocidadWidget = widgets.RoundedContainer(
            "Velocidad\n Viento", "velocidad.svg", "10 km/h"
        )
        self.direccionWidget = widgets.RoundedContainer(
...
class Thread(QThread):
    ...
    def run(self):
        ...
        schedule.every(int(self.samplingTimes["direction"])).seconds.do(
