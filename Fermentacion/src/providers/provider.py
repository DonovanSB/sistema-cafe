...
logging.basicConfig(
    filename=root + "/fermentacion.log",
...
class Data:
    ...
    def initDataService(self):
        ...
        self.envService = DataService(
            ...
            namesEnv,
            ["tempEnv", "humEnv"],
            [self.textTempR, self.textHumR],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.temp1Service = DataService(
            self.sqlite,
            "Temperatura1",
            namesTemp,
            "temp1",
        ...
        self.brixService = DataService(
            self.sqlite, "Brix", namesBrix, "brix", self.textBrix, self.numData, "Bx"
        )
        self.phService = DataService(
            self.sqlite, "PH", namesPH, "ph", self.textPh, self.numData, ""
        )
    ...
    def plot(self, datax, datay, title, index):
        try:
            ...
            self.figure.figure.subplots_adjust(
                top=0.85, bottom=0.21, left=0.1, right=0.95
            )
            self.figure.draw()
        except:
