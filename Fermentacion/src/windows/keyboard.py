import sys
import os
from PyQt5.QtWidgets import QLineEdit, QPushButton, QDesktopWidget, QGridLayout, QDialog
...
class Keyboard(QDialog):
    ...
    def createWidgets(self):
        ...
        self.display.setStyleSheet(
            "font:20px;margin-left: 30px; margin-right: 30px; padding: 5px"
        )
    ...
    def centerWindow(self):
        ...
        self.move(
            sizeScreen.x() - sizeWindow.width() / 2,
            sizeScreen.y() - sizeWindow.height() / 2,
        )
    ...
    def sendValue(self):
        try:
            ...
            self.signals.signalUpdateInputValue.emit(self.name, valueDisplay)
            self.close()
        except:
...
class Buttoms(QPushButton):
    ...
    def raisedButton(self, name):
        self.setText(name)
        self.styleButtom = (
            "QPushButton {background-color:#0d47a1; color: white; border-radius: 15px;font: 15px bold; margin: 5px 5px 5px 5px ; padding: 10px;}"
    ...
    def backspace(self):
        self.styleButtom = (
            "QPushButton {background-color:rgba(0,0,0,0); color: white; border-radius: 15px;font: 15px bold; margin: 5px 5px 5px 5px }"
    ...
    def send(self):
        self.styleButtom = (
            "QPushButton {background-color:#00c853; color: white; border-radius: 15px;font: 15px bold; margin: 5px 5px 5px 5px }"
            "QPushButton:hover { background-color: #5efc82}"
