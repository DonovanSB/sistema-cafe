...
class RoundedContainer(QFrame):
    def __init__(self, title, iconName, initialValue):
        ...
        # Estilo del Frame
        self.setStyleSheet(
            "background-color: white; border-radius:10px; margin:0px 0px 0px 0px"
        )
        ...
        self.text.setStyleSheet(
            "background: rgba(0,0,0,0);color: black; font: 20px; margin: 0px 0px 0px 0px"
        )
...
class RoundedContainerInput(QFrame):
    def __init__(self, title, iconName, initialValue):
        ...
        self.styleBottoms = """QPushButton {background-color:#0d47a1;
                                            color: white;
                                            border-radius:20px;
                                            font: 15px bold;
...
class EnvironmentWidget(QFrame):
    def __init__(self, title, initialValue1, initialValue2):
        ...
        labelTemperature.setStyleSheet(
            "background: rgba(0,0,0,0);color: black; font: 15px; font-weight:bold; margin: 10px 30px 0px 0px"
        )

        self.tempValue = QLabel(initialValue1)
        self.tempValue.setAlignment(Qt.AlignCenter)
        self.tempValue.setStyleSheet(
            "background: rgba(0,0,0,0);color: black; font: 20px; margin: 0px 0px 10px 0px"
        )

        labelHumidity = QLabel("Humedad")
        labelHumidity.setAlignment(Qt.AlignCenter)
        labelHumidity.setStyleSheet(
            "background: rgba(0,0,0,0);color: black; font: 15px; font-weight:bold; margin: 10px 30px 0px 0px"
        )
...
class MenuVisualizacion(QFrame):
    def __init__(self):
        ...
        labelSelecionar.setStyleSheet(
            "background:rgba(0,0,0,0);color:black;margin: 30px 0px 0px 0px"
        )
        labelSelecionar.setAlignment(Qt.AlignCenter)

        # ---Lista--
        self.listaVariables = QComboBox()
        self.listaVariables.setStyleSheet(
            "background:#5472d3;color:black; font:18px; padding: 5px 0px 5px 10px; selection-background-color: #0d47a1; margin: 0px 0px 0px 10px; border-radius:10px"
        )
        self.sensores = [
            "Temperatura R",
            "Humedad R",
...
class MenuBar:
    def __init__(self, menuBar):
        ...
        self.menuBar.setStyleSheet("background: #002171; color:white")
        ...
        preferenciasAction = QAction(
            "Preferencias", self.menuBar, triggered=self.showDialog
        )
