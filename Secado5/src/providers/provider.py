...
class Data:
    ...
    def initDataService(self):
        ...
        self.envService = DataService(
            ...
            namesEnv,
            ["tempEnv", "humEnv"],
            [self.textTempEnv, self.textHumEnv],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.env1Service = DataService(
            self.sqlite,
            "Zona1",
            namesEnv,
            ["temp1", "hum1"],
            [self.textTemp1, self.textHum1],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.env2Service = DataService(
            self.sqlite,
            "Zona2",
            namesEnv,
            ["temp2", "hum2"],
            [self.textTemp2, self.textHum2],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.env3Service = DataService(
            self.sqlite,
            "Zona3",
            namesEnv,
            ["temp3", "hum3"],
            [self.textTemp3, self.textHum3],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.env4Service = DataService(
            self.sqlite,
            "Zona4",
            namesEnv,
            ["temp4", "hum4"],
            [self.textTemp4, self.textHum4],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.env5Service = DataService(
            self.sqlite,
            "Zona5",
            namesEnv,
            ["temp5", "hum5"],
            [self.textTemp5, self.textHum5],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.humGrainService = DataService(
            self.sqlite,
            "HGrano",
            namesHG,
            "humGrano",
