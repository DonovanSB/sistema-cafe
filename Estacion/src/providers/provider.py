...
class Data:
    def __init__(
        self, loading, textTemp, textHum, textIrrad, textSpeed, textDir, textRain
    ):
        ...
        try:
            ...
            logging.error(
                "No se pudo encontrar la ruta de almacenamiento de datos en prefs.json"
            )
    ...
    def initDataService(self):
        ...
        self.envService = DataService(
            self.sqlite,
            "Ambiente",
            namesEnv,
            ["tempEnv", "humEnv"],
            [self.textTemp, self.textHum],
            self.numData,
            ["°C", "%"],
            2,
        )
        self.irradService = DataService(
            self.sqlite,
            "Irradiancia",
            namesIrrad,
            "irrad",
        ...
        self.directionService = DataService(
            self.sqlite,
            "DireccionV",
            namesDir,
            "direction",
    ...
    def initSQLite(self, route):
        ...
        self.sqlite.createTable(
            nameTable="Ambiente", fields="(time date, temperatura real, humedad real)"
        )
        self.sqlite.createTable(
            nameTable="Irradiancia", fields="(time date, irradiancia real)"
        )
        self.sqlite.createTable(
            nameTable="VelocidadV", fields="(time date, velocidad real)"
        )
        self.sqlite.createTable(
            nameTable="DireccionV", fields="(time date, direccion real)"
        )
    ...
    def getData(self, index):
        data = [
            self.envService.data[0],
            self.envService.data[1],
            self.irradService.data,
            self.speedService.data,
            self.directionService.data,
            self.rainService.data,
        ]
        return data[index]
    ...
    def getTime(self, index):
        timeData = [
            ...
            self.envService.time,
            self.irradService.time,
            self.speedService.time,
            self.directionService.time,
            self.rainService.time,
        ]
...
class DataService:
    def __init__(
        self, sqlite, nameTable, namesDB, name, text, numData, units, numVarSensor=1
    ):
        self.prefs = LocalStorage(route=rutaPrefsUser, name="prefs")
    ...
    def checkLimitDB(self, table, limit):
        ...
        if firstRow:
            dateAdded = QDateTime.fromString(
                firstRow[0][0], "yyyy-MM-dd hh:mm:ss"
            ).addDays(limit)
...
class Mqtt:
    ...
    def checkLimitDB(self, limit):
        ...
        if firstRow:
            dateAdded = QDateTime.fromMSecsSinceEpoch(firstRow[0][3] * 1000).addDays(
                limit
            )
            if QDateTime.currentDateTime() >= dateAdded:
...
class SQLite:
    ...
    def removeByDate(self, table, limit):
        qmutex.lock()
        self.cursor.execute(
            "DELETE FROM " + table + " WHERE time<= date('now', '-%s day')" % str(limit)
        )
        self.con.commit()
        qmutex.unlock()
...
class Plotter:
    ...
    def plot(self, datax, datay, title, index):
        try:
            ...
            self.figure.figure.subplots_adjust(
                top=0.85, bottom=0.21, left=0.13, right=0.95
            )
            self.figure.draw()
        except:
